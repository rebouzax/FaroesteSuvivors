...
export class DesertBackgroundView {
  constructor(canvas) {
    ...
    import("three/addons/loaders/GLTFLoader.js")
      ...
      .then(({scene,animations})=>{
        ...
        this.addSand();
        this.draw();
      }).catch(error=>{if(!this.disposed) console.warn("Cenário 3D do menu indisponível.",error);});
    this.sync();
  }
  addSand(){
    const geometry=new THREE.BufferGeometry(),positions=new Float32Array(90*3);
    for(let i=0;i<90;i++){
    ...
    this.grains=new THREE.Points(geometry,new THREE.PointsMaterial({color:0xffebbb,size:0.09,transparent:true,opacity:0.48}));
    this.scene.add(this.grains);
  }
  resize(){
    const width=innerWidth,height=innerHeight,aspect=width/Math.max(1,height);
  ...
  tick=(now)=>{
    ...
    if(this.grains){
      ...
      for(let i=0;i<90;i++)attr.setX(i,((attr.getX(i)+dt*(1+(i%5)*0.28)+26)%52)-26);
  ...
  sync(){
    ...
    this.last=null;
    this.draw();
    if(this.wind&&!this.motion.matches&&!document.hidden) this.frame=requestAnimationFrame(this.tick);
  }
  setWind(enabled){

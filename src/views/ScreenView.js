export class ScreenView {
  ...
  updateHud(run) {
    ...
    const hint = this.root.querySelector("#merchant-hint");
    hint.hidden = !run.merchant;
    if (run.merchant) {
      const dx = run.merchant.x - p.x,
        dz = run.merchant.z - p.z;
      const directions = ["→", "↘", "↓", "↙", "←", "↖", "↑", "↗"];
      const arrow =
        directions[(Math.round(Math.atan2(dz, dx) / (Math.PI / 4)) + 8) % 8];
      hint.textContent = `◈ Bento ${arrow} · ${Math.round(Math.hypot(dx, dz))} m · parte em ${Math.ceil(run.merchant.leavesAt - run.time)} s · aproxime-se para negociar`;
    }
